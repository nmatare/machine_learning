...
## 45.7
params = list(booster = 'gbtree', objective = 'reg:linear', nrounds = 100, max_depth = 4)

XGBST.cv <- xgb.cv( params = params, data = dt.bike.train[ ,-'count'], 
                    label = as.vector(unlist(dt.bike.train[ ,'count'])),
                    nthread = detectCores() - 1, verbose = 0,
                    nfold = 5, nrounds = 100)
